...
class FormModel{
	...
	public function check()
	{
		...
		setcookie('TOKEN', RANDOM_TOKEN, time()+3600, "/contact02/", COOKIE_DOMAIN);
		$this->post['TOKEN'] = RANDOM_TOKEN;
	...
	public function mailsend()
	{
		...
		if($_COOKIE['TOKEN'] != $this->post['TOKEN']){ die('既に送信済みか、不正な画面遷移です。'); }
		...
		setcookie('TOKEN', RANDOM_TOKEN, time()-9000, "/contact02/", COOKIE_DOMAIN);
	...
	private function errorcheck(&$post)
	{
		...
		else
		{
			...
			if(mb_strlen($post['name1']) > $strmaxlenByte ){
				$error .= "お名前の文字数オーバーです。（".$strmaxlenByte."文字以内）<br />\n";
			}
		...
		if(mb_strlen($post['kana1']) > $strmaxlenByte ){
			$error .= "ふりがなの文字数オーバーです。（".$strmaxlenByte."文字以内）<br />\n";
		}
		if(mb_strlen($post['kana2']) > $strmaxlenByte ){
			$error .= "ふりがなの文字数オーバーです。（".$strmaxlenByte."文字以内）<br />\n";
		}
		if(mb_strlen($post['sex']) > $strmaxlenByte ){
			$error .= "性別の文字数オーバーです。（".$strmaxlenByte."文字以内）<br />\n";
		}
		if(mb_strlen($post['country']) > $strmaxlenByte ){
			$error .= "国籍の文字数オーバーです。（".$strmaxlenByte."文字以内）<br />\n";
		}

		if ($post['add'] == '' || $post['add'] == '')
		{
			$error .= "住所が未入力です。<br />\n";
		}
		else
		{
			...
			if(mb_strlen($post['add']) > $strmaxlenByte ){
				$error .= "住所の文字数オーバーです。（".$strmaxlenByte."文字以内）<br />\n";
			}
			if(mb_strlen($post['add']) > $strmaxlenByte ){
				$error .= "住所の文字数オーバーです。（".$strmaxlenByte."文字以内）<br />\n";
			}
		...
		if ($post['tel'] != '')
		{
			...
			if(mb_strlen($post['tel']) > $strmaxlenByte){
				$error .= "TELの文字数オーバーです。（".$strmaxlenByte."文字以内）<br />\n";
			}
		}
		...
		else
		{
			...
			if(mb_strlen($post['email']) > $strmaxlenByte){
				$error .= "メールアドレスの文字数オーバーです。（".$strmaxlenByte."文字以内）<br />\n";
			}
	
			if (!preg_match('/^([a-z0-9_]|\-|\.|\+)+@(([a-z0-9_]|\-)+\.)+[a-z]{2,6}$/i',$post['email']))
			{
				$error .= "メールアドレスの書式に誤りがあります。<br />\n";
			}
		}

		if (!isset($post['email2']) || $post['email2'] == '')
		{
			$error .= "メールアドレス（再）が未入力です。<br />\n";
		}

		if (!isset($post['comment']) || $post['comment'] == '')
		{
			$error .= "お問合せ内容が未入力です。<br />\n";
		}
		else
		{
			$strmaxlenByte = 2000;
			if(mb_strlen($post['comment']) > $strmaxlenByte){
				$error .= "お問合せ内容の文字数オーバーです。（".$strmaxlenByte."文字以内）<br />\n";
			}
		}
